...
int posfir[Max], poslas[Max];

void build(int x, node * root)
{
    root->val = x;
    vis[x] = true;
    int lson = first[posfir[x] + 1];
    int rson = last[poslas[x] - 1];
    if (lson == 0 || rson == 0 || vis[lson] || vis[rson])
    {
        print(x); print(' ');
        return;
    }
    root->lson = new node();
    root->rson = new node();
    build(lson, root->lson);
    print(root->val); print(' ');
    build(rson, root->rson);
}
