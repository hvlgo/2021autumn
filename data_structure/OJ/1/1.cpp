...
namespace fastIO {
#define BUF_SIZE 100000//缓存区大小
#define DECIMAL 6   //double 类型的 默认输出保留位数
#define LL long long
...
{
    node() {}
    node(int _id, node * _pre = nullptr, node * _succ = nullptr) : id(_id), pre(_pre), succ(_succ) {}
    int id;
    node * pre;
    node * succ;
};
...
linkList::linkList()
{
    header = new node;
    tail = new node;
    header->pre = nullptr, header->succ = tail;
    tail->pre = header, tail->succ = nullptr;
    _size = 0, pos = 0;
}

int linkList::position()
{
    return (pos = (pos + m - 2) % _size + 1);
}

void linkList::insert(node *e)
{
    if (pos == 0)
    {
        header->succ = e;
        tail->pre = e;
        e->pre = header, e->succ = tail;
        pos = 1, _size++;
        return;
    }
    node * tmp = header;
    ...
    e->pre = tmp, e->succ = tmp->succ;
    tmp->succ->pre = e, tmp->succ = e;
    pos = s + 1, _size++;
}
