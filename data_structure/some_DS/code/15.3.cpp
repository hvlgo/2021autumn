...
int main(){
	...
	for(int i = 1;i <= na;i++){
		int cmd = RD(),x = RD();
		if(cmd == 1)printf("%d\n",get_rank(root,x));//函数都写好了，注意：需要递归的函数都从根开始，不需要递归的函数直接查询
		else if(cmd == 2)printf("%d\n",get_val(root,x + 1));
		else if(cmd == 3)printf("%d\n",get_pre(x));//注意：因为初始化时插入了INF和-INF,所以查询排名时要减1(-INF不是第一小，是“第零小”)
		else if(cmd == 4)printf("%d\n",get_next(x));//同理，用排名查询值得时候要查x + 1名，因为第一名(其实不是)是-INF
		else if(cmd == 5)insert(root,x);
		}
	return 0;
	}